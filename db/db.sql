...
CREATE TABLE User (
    user_id INT NOT NULL AUTO_INCREMENT, -- user_id自动加一
    ...
    avatar VARCHAR(255) NOT NULL, -- 用户头像的url 
    PRIMARY KEY (user_id)
);
...
CREATE TABLE following_table (
    ...
    FOREIGN KEY (attention) REFERENCES User(user_id),
    FOREIGN KEY (fans) REFERENCES User(user_id)
);